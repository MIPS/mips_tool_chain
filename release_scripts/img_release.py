...
targets = [ "mips-img-elf", "mips-img-linux-gnu" ]
...
target_to_name = { "mips-img-elf" : "IMG.Bare.Metal",
		   "mips-img-linux-gnu" : "IMG.Linux"}

target_to_var = { "mips-img-elf" : "MME",
		  "mips-img-linux-gnu" : "MMLG"}

scriptpath=os.path.dirname(os.path.realpath(__file__))

if True:
  f = open("%s/template/downloads.tmpl" % scriptpath, mode='r')
  template = f.read()
  f.close()
  ...
  for target in targets:
    for host in ["i686-pc-linux-gnu", "i686-w64-mingw32", "x86_64-pc-linux-gnu", "x86_64-w64-mingw32"]:
      if not os.path.exists ("%s_%s.tgz" % (target, host)):
        ...
        sys.exit(1)
      filename = "Codescape.GNU.Tools.Package.%s.for.MIPS.%s.%s.tar.gz" % (version, target_to_name[target], host_to_name[host])
      if os.path.exists (filename):
        print ("ERROR: Toolkit tarball destination already exists %s" % filename)
        sys.exit(1)

  components = ["binutils", "gdb", "newlib", "uclibc", "gcc", "glibc", "smallclib", "gold", "qemu", "packages", "python"]
  ...
  f = open("%s/template/downloads.html" % scriptpath, 'w')
  f.write(template)
  f.close()
  ...
  f = open("%s/template/downloads.tmpl" % scriptpath, 'w')
  f.write(template)
  f.close()

if True:
  f = open("%s/template/releasenotes.html" % scriptpath, mode='r')
  template = f.read()
  f.close()
  if relnotes:
  ...
  if (os.path.exists ("docs")):
    ...
    for doc in os.listdir ("docs"):
      if fnmatch.fnmatch(doc, '*.pdf'):
        ...
        dver = (dver.split ("DN", 1))[0].replace('_','.').lstrip('0')
  ...
  f = open("%s/template/releasenotes.html" % scriptpath, 'w')
  f.write(template)
  f.close()

if True:
  templatedir = os.path.join (scriptpath, "template")
  ...
  mipsswrel_path="/projects/mipsswrel/toolchains"
  ...
  if os.access(mipsswinst_path, os.X_OK | os.W_OK):
    print("Extracting x86_64 elf toolchain to " + os.path.join(mipsswinst_path, 'mips-img-elf', version))
    tf = tarfile.open("Codescape.GNU.Tools.Package.%s.for.MIPS.IMG.Bare.Metal.CentOS-6.x86_64.tar.gz" % version,
                    'r|gz')
    if tf:
      tf.extractall(mipsswinst_path)
      subprocess.call (["chmod", "-R", "g+w,o-w", os.path.join(mipsswinst_path, 'mips-img-elf', version)])

    print("Extracting x86_64 linux toolchain to " + os.path.join(mipsswinst_path, 'mips-img-linux-gnu', version))
    tf = tarfile.open("Codescape.GNU.Tools.Package.%s.for.MIPS.IMG.Linux.CentOS-6.x86_64.tar.gz" % version,
                      'r|gz')
    if tf:
      tf.extractall(mipsswinst_path)
      subprocess.call (["chmod", "-R", "g+w,o-w", os.path.join(mipsswinst_path, 'mips-img-linux-gnu', version)])

  else:
    print("No write permissions on %s, not installing x86_64 toolchains" % mipsswinst_path)

  print("Adding website changes to git")
  ret=subprocess.Popen(["git", "add", "template"], cwd = scriptpath)
  if ret == 0:
    ret=subprocess.Popen(["git", "commit", "-m", "\"MIPS IMG toolchain release " + version + "\""], cwd = scriptpath)
  if ret != 0:
