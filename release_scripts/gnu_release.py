...
targets = [ "mips-mti-elf", "mips-mti-linux-gnu" ]
...
target_to_name = { "mips-mti-elf" : "MTI.Bare.Metal",
		   "mips-mti-linux-gnu" : "MTI.Linux"}

target_to_var = { "mips-mti-elf" : "MME",
		  "mips-mti-linux-gnu" : "MMLG"}
...
if True:
  ...
  if os.access(mipsswinst_path, os.X_OK | os.W_OK):
    print("Extracting x86_64 elf toolchain to " + os.path.join(mipsswinst_path, 'mips-mti-elf', version))
    tf = tarfile.open("Codescape.GNU.Tools.Package.%s.for.MIPS.MTI.Bare.Metal.CentOS-6.x86_64.tar.gz" % version,
                    'r|gz')
    if tf:
    ...
    if tf:
      tf.extractall(mipsswinst_path)
      subprocess.call (["chmod", "-R", "g+w,o-w", os.path.join(mipsswinst_path, 'mips-mti-linux-gnu', version)])
  ...
  if ret == 0:
    ret=subprocess.Popen(["git", "commit", "-m", "\"MIPS MTI toolchain release " + version + "\""], cwd = scriptpath)
  if ret != 0:
